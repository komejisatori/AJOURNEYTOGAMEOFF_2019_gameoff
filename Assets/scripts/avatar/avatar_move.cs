    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (m_create)
            {
                ...
                {
                    this.transform.position = new Vector3(this.transform.position.x, m_terrianlist[0].gameObject.transform.position.y, this.transform.position.z);
                    m_start_pos = 0;
                }
                if (Input.GetKeyDown(KeyCode.S))
                {
                    this.transform.position = new Vector3(this.transform.position.x, m_terrianlist[1].gameObject.transform.position.y, this.transform.position.z);
                    m_start_pos = 1;
                }
                if (Input.GetKeyDown(KeyCode.Return))
                {
                    ...
                    foreach(terrian terr in m_terrianlist)
                    {
                        terr.GiveGravity();
                    }
                    //Debug.Log("start");
